...
public sealed class TheNameProperty
{
    ...
    public async Task ShouldBeAppendedToTheSendersOfLogMessages()
    {
        ...
        logMessage.Senders[0].Should().Be("ChildLogger", because: "the child logger's name should be the first sender in the log message's senders.");
        logMessage.Senders[1].Should().Be(loggerName, because: "the logger's name should be the second sender in the log message's senders.");
    }
}
